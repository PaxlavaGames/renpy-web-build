...
init python:
    ...
    def filters_match(path_filters, path, path_type):
        ...
        for (f_rule, f_type, f_pattern) in path_filters:
            if path_type == f_type and  match(path, f_pattern):
                return f_rule
        return False
    ...
    def build_web(p, gui=True):
        ...
        Distributor(p, packages=[ "web" ], packagedest=os.path.join(destination, "game"),
    ...
    renpy.arguments.register_command("custom_web_build_command", custom_web_build_command)